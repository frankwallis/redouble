...
describe(
  ...
  () => {
    ...
    describe(
      ...
      () => {
        test(
          "returns None whe no-one has bid",
          () => {
        ...
        test(
          "returns Call",
          () => {
            let board = Board.create(Seat.West) |> Board.makeBid(Call(2, BidSuit.Hearts));
            expect(lastBid(board))
            |> toEqual(Some({seat: Seat.West, bid: Call(2, BidSuit.Hearts)}))
          }
        );
        test(
          "returns Double",
    ...
    describe(
      ...
      () => {
        test(
          "returns None when no-one has bid",
          () => {
            let board = Board.create(Seat.West);
        ...
        test(
          "returns None after a NoBid",
          () => {
            let board = Board.create(Seat.West) |> Board.makeBid(NoBid);
            expect(lastCall(board)) |> toEqual(None)
          }
        );
        test(
          ...
          () => {
            let board =
              ...
              |> Board.makeBid(NoBid)
              |> Board.makeBid(Call(2, BidSuit.Hearts));
            expect(lastCall(board))
            |> toEqual(Some({seat: Seat.North, bid: Call(2, BidSuit.Hearts)}))
          }
        );
        test(
          "returns Call after a Double",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(NoBid)
        ...
        test(
          "returns last Call which was made",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(NoBid)
            ...
            expect(lastCall(board))
            |> toEqual(Some({seat: Seat.South, bid: Call(3, BidSuit.Spades)}))
          }
        )
      }
    );
    describe(
      ...
      () => {
        test(
          "returns None before the bidding starts",
          () => {
            let board = Board.create(Seat.West);
    ...
    describe(
      ...
      () => {
        test(
          "returns false when there has been no bidding",
          () => {
            let board = Board.create(Seat.West);
        ...
        test(
          "returns false when there have only been 2 NoBids after a Call",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(Call(2, BidSuit.Hearts))
        ...
        test(
          "returns true when there have been 3 NoBids after a Call",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(Call(2, BidSuit.Hearts))
        ...
        test(
          "returns false when there have been 3 NoBids initially",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(NoBid)
        ...
        test(
          "returns true when there are four NoBids on the first round",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(NoBid)
    ...
    describe(
      ...
      () => {
        test(
          "returns None when bidding has not ended",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(Call(2, BidSuit.Hearts))
        ...
        test(
          "returns None when hand thrown in",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(NoBid)
        ...
        test(
          "returns the suit of the bid contract",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(Call(2, BidSuit.Hearts))
        ...
        test(
          "returns the suit of the bid contract for doubled NoTrump",
          () => {
            let board =
              Board.create(Seat.West)
    ...
    describe(
      ...
      () => {
        ...
        test(
          "returns the person who bid the contract suit first",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(NoBid)
        ...
        test(
          "returns the person on declaring team who bid the contract suit first",
          () => {
            let board =
              Board.create(Seat.West)
              |> Board.makeBid(Call(1, BidSuit.Hearts))
