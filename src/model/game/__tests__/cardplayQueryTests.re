...
describe "CardplayQuery" (fun () => {
  ...
  describe "playHasEnded" (fun () => {
    test "returns false when the bidding has not ended" (fun () => {
      let board = Board.fromPBN "W:AK76.QJ5.T94.832 832.AK76.QJ5.T94 T94.832.AK76.QJ5 QJ5.T94.832.AK76"
      |> Board.makeBids [(Call 1 BidSuit.Spades), NoBid, NoBid];
      expect (playHasEnded board) |> toEqual (false);
    });

    test "returns false when some cards have been played" (fun () => {
      let board = Board.fromPBN "W:AK76.QJ5.T94.832 832.AK76.QJ5.T94 T94.832.AK76.QJ5 QJ5.T94.832.AK76"
      |> Board.makeBids [(Call 1 BidSuit.Spades), NoBid, NoBid]
      |> Board.playTrick [(Pip.Queen, Suit.Spades), (Pip.Ace, Suit.Spades), (Pip.Two, Suit.Spades), (Pip.Four, Suit.Spades)];
      expect (playHasEnded board) |> toEqual (false);
    });

    test "returns true when all cards have been played" (fun () => {
      let board = Board.fromPBN "W:A... 2... 4... Q..."
      |> Board.makeBids [(Call 1 BidSuit.Spades), NoBid, NoBid]
      |> Board.playTrick [(Pip.Queen, Suit.Spades), (Pip.Ace, Suit.Spades), (Pip.Two, Suit.Spades), (Pip.Four, Suit.Spades)];
