  ...
  if (biddingHasEnded(board)) {
    ...
    | Bid.Double =>
      ...
      | Some({seat: lastCaller, bid: Bid.Call(_, _)}) =>
        Seat.isPartner(lastCaller, bidder) ? Some("You cannot double your partner!") : None
    ...
    | Bid.Call(level, _suit) =>
      if (level < 1 || level > 7) {
        ...
        | Some({seat: _, bid: Call(_, _) as lastBid}) when Bid.compare(bid, lastBid) <= 0 =>
          Some("Bid must be higher than " ++ Bid.description(lastBid))
        | _ => None
        }
      }
    }
...
let makeBids = (bids, board) =>
  bids |> List.fold_left((result, bid) => makeBid(bid, result), board);
...
let playTrick = (cards, board) =>
  cards |> List.fold_left((result, card) => playCard(card, result), board);
