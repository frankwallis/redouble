...
describe("DdsStrategy", () => {
  open Bid;
  open Card;

  describe("getCard", () => {
    testPromise("observes current trick", () => {
      ...
      Js.Promise.(
        DdsStrategy.getCard(board)
          |> then_ (card => expect(card) |> toEqual((Pip.Two, Suit.Spades)) |> resolve)
      )
    });

    testPromise("unblocks in 3 card ending", () => {
      let board = Board.fromPBN("W:456... 2...A2 7...76 A.A..3")
        |> Board.makeBids([NoBid, NoBid, NoBid, Call(1, BidSuit.NoTrumps), NoBid, NoBid, NoBid]);

      Js.Promise.(
        playAll(board)
          |> then_ (board => expect(Board.declarerTricks(board)) |> toEqual(3) |> resolve)
      )
    });

    testPromise("Bond beats Drax", () => {
      ...
      Js.Promise.(
        playAll(board)
          |> then_ (board => expect(Board.declarerTricks(board)) |> toEqual(13) |> resolve)
      )
    });

    testPromise("Solves the PotBoiler", () => {
