...
let reducer state action =>
  ...
  | JumpBack => {...state, position: state.position < (List.length state.history) -1 ? state.position +1 : state.position, autoPlay: false, sequenceNo: state.sequenceNo +1 }
...
let canResume state => not state.autoPlay;
