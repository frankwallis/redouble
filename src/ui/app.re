...
let make = (~state: Store.appState, ~dispatch, _children) => {
  ...
  let makeBid = (bid) =>
    dispatch(ReduxThunk.Thunk(GameActions.makeBid(bid, state.game.sequenceNo)));
  let playCard = (card) =>
    dispatch(ReduxThunk.Thunk(GameActions.playCard(card, state.game.sequenceNo)));
